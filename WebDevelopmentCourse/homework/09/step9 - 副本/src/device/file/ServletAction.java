...
public class ServletAction extends HttpServlet {
	...
	private void exportDeviceRecord(HttpServletRequest request, HttpServletResponse response, JSONObject json) throws JSONException, SQLException, IOException {
		...
		Data data = getPageParameters(request, response);
		dao.getDeviceRecord(data, json);
		getExportDeviceRecordToFile(data, json);
		getExportDeviceRecordToTxt(data, json);
		getExportDeviceRecordToExcel(data, json);
		getExportDeviceRecordToPdf(data, json);
		/*========================================上传文件函数 结束========================================*/
	}

	private void getExportDeviceRecordToPdf(Data data, JSONObject json) {
	}


	private void getExportDeviceRecordToTxt(Data data, JSONObject json) {
	}

	private void getExportDeviceRecordToFile(Data data, JSONObject json) throws JSONException {
		String jsonStr = json.toString();
		File jsonFile = new File("D:\\upload\\export_device.txt");
		json.put("download_url","/upload/export_device.txt");
		try {
			// 文件不存在就创建文件
			if (!jsonFile.exists()) {
	...
	private void getExportDeviceRecordToExcel(Data data, JSONObject json) throws JSONException, IOException {
		MyExcel me=new MyExcel("D:\\upload\\export_device.xls");
		json.put("download_url","/upload/export_device.xls");
		json.put("file_path","D:\\upload\\export_device.xls");
