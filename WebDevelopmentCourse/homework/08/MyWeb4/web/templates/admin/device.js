...
var Page = function () {
    ...
    var showMenuRecord = function(res) {
        var html = "";
        var list = res.aaData;
        var sub1 = false;
        var sub2 = false;
            ...
            if (json.result_code == 0) {
                ...
                if (list != undefined && list.length > 0) {
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html = html + "<td>" + i + "</td>";
                        html = html + "<td>" + record.device_id + "</td>";
                        html = html + "<td>" + record.device_name + "</td>";
                        html = html + "<td>" + record.creator + "</td>";
                        html = html + "<td>" + record.creation_time + "</td>";
                        html = html + "<td>";
                        ...
                        html = html + "</td>";
                        // html = html + "<td>" + record.company_id + "</td>";
                        html = html + "<a href=\"javascript:Page.onModifyRecord(" + record.id + ")\">【修改记录】</a>";
                        html = html + "</tr>";
                    }
                }
    ...
    var getDeviceRecordDataTable = function () {
        $('.datatable').dataTable({
            ...
            "aoColumns": [{
                ...
                "mRender": function (data, type, full) {
                    // sReturn = '<div>' + full.company_id + '</div>';
                    // return sReturn;
                    sReturn = '<a href="' + full.company_id + '" target="_blank">查看图片</a>';
